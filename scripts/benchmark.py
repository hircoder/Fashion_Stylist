...
def main():
    ...
    base = replace(Settings.from_env(), index_dir=Path(args.index_dir))
    before = rss_mb()
    t = time.perf_counter()
    index = SearchIndex.load(base.index_dir, expected_model=base.embedding_name)
    load_s = time.perf_counter() - t
    embedder = make_embedder(base)
    svc = RecommendationService(index, embedder, base, llm=None)
    asyncio.run(svc.recommend(RecommendRequest(query="warm up", use_llm=False)))
    after = rss_mb()
    out = {
    ...
    for c in [int(x) for x in args.concurrency.split(",")]:
        out["runs"].append(asyncio.run(run(svc, c, args.requests)))
