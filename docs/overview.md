...
* Indexing only the popular rows is a bias, so it is never hidden: every response names the subset it was answered from (`index_info.sampling`), and the evaluation slide runs the same queries against a random sample and the full catalog to measure what the subset costs.
